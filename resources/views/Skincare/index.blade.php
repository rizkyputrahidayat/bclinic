...
@section('content')
    ...
    <table class="table">
        ...
        <tbody>
            @if (!empty($Skincare))
                @foreach ($Skincare as $key => $item)
                    <tr>
                        <th scope="row">{{ ++$key }}</th>
                        <td>{{ $item->title }}</td>
                        <td>{{ $item->description }}</td>
                        <td>
                            <img style="height: 50px; width:50px;" src="{{ asset('image/Skincare/' . $item->image) }}">
                        </td>
                        <td>
                            <button type="button" class="btn btn-warning mb-1" data-toggle="modal"
                                data-target="#editModalSlimming">Edit</button>

                            <form action="{{ route('Skincare.destroy', $item->id) }}" method="POST">
                                @csrf
                                @method('DELETE')
                            ...
                            <div class="modal fade" id="editModalSlimming" tabindex="-1" role="dialog"
                                ...
                                <div class="modal-dialog" role="document">
                                    <div class="modal-content">
                                        ...
                                        <form action="{{ route('Skincare.update', $item->id) }}" method="POST"
    ...
    <div class="modal" id="tutorialModal" tabindex="-1" role="dialog">
        <div class="modal-dialog modal-md" role="document">
            <div class="modal-content">
                <div class="modal-header">
                    <h5 class="modal-title">Add New Skincare</h5>
                    <button type="button" class="close" data-dismiss="modal" aria-label="Close">
                        <span aria-hidden="true">&times;</span>
                    </button>
                </div>
